...
export const SocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // There is no Socket.IO server on Next.js/Vercel (serverless), so realtime is
    // disabled unless a dedicated socket server URL is provided. Notifications are
    // still persisted server-side and appear on the next fetch/refresh.
    const socketUrl = process.env.NEXT_PUBLIC_SOCKET_URL || null;
