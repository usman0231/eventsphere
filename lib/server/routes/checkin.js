...
router.post('/:id/undo', protect, authorize('admin', 'organizer'), async (req, res) => {
  try {
    ...
    await logActivity({ user: req.user._id, action: 'Check-In Undone', entity: 'Registration', entityId: registration._id, details: `${registration.user.name} → ${expo.title}${reason ? ` (${reason})` : ''}`, req });
