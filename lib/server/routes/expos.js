...
router.get('/', async (req, res) => {
  try {
    ...
    // Only show admin-approved expos publicly. $nin also matches legacy expos
    // that predate the approval field (no approvalStatus set).
...
// GET /api/expos/pending — admin queue of expos awaiting approval.
router.get('/pending', protect, authorize('admin', 'superadmin'), async (req, res) => {
  try {
    const expos = await Expo.find({ approvalStatus: 'pending' })
...
router.post('/', protect, authorize('admin', 'organizer'), async (req, res) => {
  try {
    ...
    const expo = await Expo.create({
      ...
      // Admin-created expos are trusted and auto-approved; organizer-created
      // expos await admin approval and stay as drafts until then.
      approvalStatus: isAdmin ? 'approved' : 'pending',
      approvedBy: isAdmin ? req.user._id : undefined,
      approvedAt: isAdmin ? new Date() : undefined,
      status: isAdmin ? (req.body.status || 'draft') : 'draft',
    });
...
router.delete('/:id', protect, authorize('admin', 'organizer'), async (req, res) => {
  try {
    ...
    // Cascade: remove everything tied to this expo so no orphaned records are
    // left behind (which previously broke the attendance/undo-check-in screen).
    await Promise.all([
      Booth.deleteMany({ expo: req.params.id }),
      Session.deleteMany({ expo: req.params.id }),
      Sponsor.deleteMany({ expo: req.params.id }),
      Review.deleteMany({ expo: req.params.id }),
      Registration.deleteMany({ expo: req.params.id }),
      Attendance.deleteMany({ expo: req.params.id }),
      ExhibitorApplication.deleteMany({ expo: req.params.id }),
    ]);
    ...
    res.json({ success: true, message: 'Expo and all related data deleted successfully' });
  } catch (err) {
    res.fail(err);
  }
});

// PUT /api/expos/:id/approve — admin approves a pending expo; it goes live.
router.put('/:id/approve', protect, authorize('admin', 'superadmin'), async (req, res) => {
  try {
    ...
    expo.rejectionReason = undefined;
    expo.approvedBy = req.user._id;
    expo.approvedAt = new Date();
    // Publish it on approval (unless the organizer already moved it further).
    if (['draft', 'published'].includes(expo.status)) expo.status = 'published';
    await expo.save();

    await logActivity({ user: req.user._id, action: 'Expo Approved', entity: 'Expo', entityId: expo._id, details: expo.title, req });

    const io = req.app.get('io');
    await createNotification(io, {
      recipient: expo.organizer,
      sender: req.user._id,
      type: 'expo_update',
      title: 'Expo approved ✅',
      message: `Your expo "${expo.title}" has been approved and is now live.`,
      link: `/expos/${expo._id}`,
      expo: expo._id,
    });
...
// PUT /api/expos/:id/reject — admin rejects a pending expo with a reason.
router.put('/:id/reject', protect, authorize('admin', 'superadmin'), async (req, res) => {
  try {
    ...
    expo.rejectionReason = (reason || '').trim() || 'No reason provided';
    expo.approvedBy = req.user._id;
    expo.approvedAt = new Date();
    await expo.save();

    await logActivity({ user: req.user._id, action: 'Expo Rejected', entity: 'Expo', entityId: expo._id, details: `${expo.title} — ${expo.rejectionReason}`, req, status: 'warning' });
    ...
    await createNotification(io, {
      ...
      sender: req.user._id,
      type: 'expo_update',
      title: 'Expo needs changes ⚠️',
      message: `Your expo "${expo.title}" was not approved: ${expo.rejectionReason}`,
      link: `/expos/${expo._id}`,
      expo: expo._id,
    });
