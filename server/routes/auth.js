...
router.post('/register', registerRules, validate, async (req, res) => {
  try {
    ...
    await logActivity({ user: user._id, action: 'User Registered', entity: 'User', entityId: user._id, details: `Role: ${user.role}`, req });

    // Send verification email. If SMTP isn't configured (dev), auto-verify so the system stays usable.
    ...
    if (autoVerified) {
      // Dev fallback: log them straight in
      const token = signToken(user._id);
      return res.status(201).json({
        success: true,
        token,
        user,
        message: 'Account created (SMTP not configured — email verification skipped)',
      });
    }
...
router.post('/resend-verification', resendVerificationRules, validate, async (req, res) => {
  try {
    ...
    sendVerificationEmail(user, rawToken).catch(err => console.error('Verification resend failed:', err.message));
...
router.post('/login', loginRules, validate, async (req, res) => {
  try {
    ...
    if (!user || !(await user.matchPassword(password))) {
      await logActivity({ action: 'Login Failed', entity: 'User', details: `Email: ${email}`, req, status: 'failed' });
      return res.status(401).json({ success: false, message: 'Invalid credentials' });
    }
    if (user.isActive === false) {
      await logActivity({ user: user._id, action: 'Login Blocked (Suspended)', entity: 'User', entityId: user._id, req, status: 'failed' });
    ...
    if (user.isEmailVerified === false) {
      await logActivity({ user: user._id, action: 'Login Blocked (Unverified)', entity: 'User', entityId: user._id, req, status: 'failed' });
    ...
    await user.save();
    const token = signToken(user._id);
    await logActivity({ user: user._id, action: 'User Login', entity: 'User', entityId: user._id, req });
...
router.delete('/users/:id', protect, authorize('admin'), async (req, res) => {
  try {
    ...
    await logActivity({ user: req.user._id, action: 'User Deleted', entity: 'User', entityId: req.params.id, details: `${user.name} (${user.email})`, req, status: 'warning' });
