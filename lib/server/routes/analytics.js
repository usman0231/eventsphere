...
router.get('/dashboard', protect, authorize('admin', 'organizer'), async (req, res) => {
  try {
    ...
    const [totalExpos, totalUsers, totalBooths, totalApplications, totalAttendance] = await Promise.all([
    ...
    const recentExpos = await Expo.find(expoFilter)
      .sort({ createdAt: -1 })
      .limit(5)
    ...
    const userRoles = isOrganizer ? [] : await User.aggregate([
      { $group: { _id: '$role', count: { $sum: 1 } } },
    ]);

    const applicationStats = await ExhibitorApplication.aggregate([
      ...(scopedExpoIds ? [{ $match: { expo: { $in: scopedExpoIds } } }] : []),
      { $group: { _id: '$status', count: { $sum: 1 } } },
    ]);
    ...
    const topExpos = await Attendance.aggregate([
      { $match: attendanceMatch },
    ...
    const attendeeRoles = await Attendance.aggregate([
      { $match: attendanceMatch },
    ...
    const revenueAgg = await Attendance.aggregate([
      { $match: attendanceMatch },
