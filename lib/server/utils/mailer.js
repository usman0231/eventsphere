...
async function sendVerificationEmail(user, code) {
  return sendMail({
    to: user.email,
    subject: `${code} is your EventSphere verification code`,
    html: wrap('Confirm your email address', `
      <p>Hi ${user.name},</p>
      <p>Thanks for signing up for EventSphere! Enter this 6-digit code on the sign-up page to verify your email. It expires in <strong>24 hours</strong>.</p>
      <div style="margin:24px 0;text-align:center;">
        <div style="display:inline-block;font-family:Arial,Helvetica,sans-serif;font-size:34px;font-weight:800;letter-spacing:10px;color:#6c3de8;background:#f3f0ff;border:1px solid #e0d8ff;border-radius:10px;padding:16px 28px;">${code}</div>
      </div>
      <p style="font-size:13px;color:#888;">If you didn't create an EventSphere account, you can safely ignore this email.</p>
    `),
    text: `Your EventSphere verification code is: ${code}\n\nEnter it on the sign-up page to verify your email. It expires in 24 hours.\n\nIf you didn't sign up, ignore this email.`,
  });
}
