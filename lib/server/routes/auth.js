...
router.put('/updateprofile', protect, async (req, res) => {
  try {
    const { name, phone, company, bio, avatar, website, category,
            foundedYear, companySize, social, notificationPrefs } = req.body;
    // Only set keys that were actually provided, so a profile save doesn't wipe
    // fields managed by a different tab (e.g. saving prefs shouldn't clear bio).
    const updates = {};
    for (const [k, v] of Object.entries({ name, phone, company, bio, avatar, website, category, foundedYear, companySize, social, notificationPrefs })) {
...
// Self-service account deactivation — requires the current password as confirmation.
// Sets isActive=false; the `protect` middleware then blocks any future login/session.
